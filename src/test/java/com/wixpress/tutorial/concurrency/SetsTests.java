...
public class SetsTests {
    ...
    public static void printStats() {
        for (Stats stat: stats) {
            System.out.println(String.format("%50s (%s) - time: %,d mSec, avgReaderTime: %,9d nanoSec, avgWriterTime: %,9d nanoSec", stat.methodName,
    ...
    private Runnable writer(final Set<Integer> theList) {
        return new Runnable() {
            @Override
            public void run() {
    ...
    private Runnable reader(final Set<Integer> theList) {
        return new Runnable() {
            @Override
            public void run() {
                long s = System.nanoTime();
